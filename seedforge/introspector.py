"""Чтение схемы БД: таблицы, колонки, типы, FK, constraints.
...
# Системные таблицы которые не нужно заполнять данными
SYSTEM_TABLES = {
    "_prisma_migrations",
...
def create_introspector(db_url: str) -> "Introspector":
    """Фабрика — создаёт нужный introspector по URL."""
...
class Introspector:
    """Базовый класс для introspection."""
...
class MySQLIntrospector(Introspector):
    ...
    def get_tables(self, schema: str = "") -> dict[str, TableInfo]:
        ...
        for row in cur.fetchall():
            ...
            # MySQL ENUM — парсим из column_type: enum('a','b','c')
            enum_values = None
